...
@manager.route("/datasets/<dataset_id>/documents/change_status", methods=["POST"])  # noqa: F821
@token_required
def change_status(tenant_id, dataset_id):
    """
    Update the status of documents in a dataset.
    ---
    tags:
      - Documents
    security:
      ...
      - in: body
        ...
        schema:
          ...
          properties:
            ...
            status:
              type: string
              description: Status to set ("1" for enable, "0" for disable).
    responses:
      200:
    ...
    if status not in ["0", "1"]:
        return get_result(message='"Status" must be either 0 or 1!', code=settings.RetCode.ARGUMENT_ERROR)

    if not doc_ids:
        return get_result(message="doc_ids cannot be empty", code=settings.RetCode.ARGUMENT_ERROR)
    ...
    for doc_id in doc_ids:
        ...
        try:
            ...
            if doc.kb_id != dataset_id:
                 result[doc_id] = {"error": "Document not in this dataset."}
                 continue
            ...
            if not settings.docStoreConn.update({"doc_id": doc_id}, {"available_int": status_int}, search.index_name(tenant_id), doc.kb_id):
